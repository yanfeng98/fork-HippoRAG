...
class OpenIE:
    ...
    def batch_openie(self, chunks: Dict[str, ChunkInfo]) -> Tuple[Dict[str, NerRawOutput], Dict[str, TripleRawOutput]]:
        ...
        chunk_passages: dict[str, str] = {chunk_key: chunk["content"] for chunk_key, chunk in chunks.items()}
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for future in pbar:
                result: NerRawOutput = future.result()
        ...
        with ThreadPoolExecutor() as executor:
            ...
            for future in pbar:
                result: TripleRawOutput = future.result()
        ...
        ner_results_dict: dict[str, NerRawOutput] = {res.chunk_id: res for res in ner_results_list}
        triple_results_dict: dict[str, TripleRawOutput] = {res.chunk_id: res for res in triple_results_list}
    ...
    def ner(self, chunk_key: str, passage: str) -> NerRawOutput:
        ...
        try:
            ...
            if metadata['finish_reason'] == 'length':
                real_response: str = fix_broken_generated_json(raw_response)
            else:
                real_response: str = raw_response
            extracted_entities: list[str] = _extract_ner_from_response(real_response)
            unique_entities: list[str] = list(dict.fromkeys(extracted_entities))
    ...
    def triple_extraction(self, chunk_key: str, passage: str, named_entities: List[str]) -> TripleRawOutput:
        ...
        try:
            ...
            if metadata['finish_reason'] == 'length':
                ...
                real_response: str = raw_response
            extracted_triples: list[str] = _extract_triples_from_response(real_response)
            triplets: list[list[str]] = filter_invalid_triples(triples=extracted_triples)
