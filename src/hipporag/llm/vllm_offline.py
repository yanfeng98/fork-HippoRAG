...
def convert_text_chat_messages_to_strings(messages: List[TextChatMessage],
                                          tokenizer: PreTrainedTokenizer,
                                          add_assistant_header=True) -> List[str]:
    return tokenizer.apply_chat_template(conversation=messages, tokenize=False)


def convert_text_chat_messages_to_input_ids(messages: List[TextChatMessage],
                                            tokenizer: PreTrainedTokenizer,
                                            add_assistant_header=True) -> List[List[int]]:
...
class VLLMOffline:
    ...
    def __init__(self, global_config, cache_dir=None, cache_filename=None, max_model_len=4096, **kwargs):
        ...
        self.client = LLM(model=model_name,
    ...
    def infer(self, messages: List[TextChatMessage], max_tokens=2048):
        ...
        vllm_output = self.client.generate(prompt_token_ids=prompt_ids,
